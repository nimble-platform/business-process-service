...
public class ProcessDAOUtility {
    ...
    public static ProcessDAO findByProcessID(String processId) {
        List<ProcessDAO> results = new JPARepositoryFactory().forBpRepository(true).getEntities(QUERY_GET_BY_PROCESS_ID, new String[]{"processId"}, new Object[]{processId});
        if(results.size() == 0) {
            return null;
        }
    ...
    public static List<ProcessDAO> getProcessDAOs() {
        List<ProcessDAO> resultSet = new JPARepositoryFactory().forBpRepository(true).getEntities(ProcessDAO.class);
        return resultSet;
    }
}
