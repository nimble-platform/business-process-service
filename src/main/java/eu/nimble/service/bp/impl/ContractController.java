...
import eu.nimble.service.bp.impl.clause.*;
...
public class ContractController {
    ...
    public ResponseEntity<ClauseType> getClauseDetails(@PathVariable(value = "documentId", required = true) String documentId,
                                                       @RequestParam(value = "clauseType", required = true) eu.nimble.service.bp.impl.clause.ClauseType clauseType) {
        try {
    ...
    public ResponseEntity addDocumentClauseToContract(@PathVariable(value = "documentId") String documentId,
        ...
        try {
            ...
            // check clause type
            eu.nimble.service.bp.impl.clause.ClauseType clauseTypeEnum;
            try {
                clauseTypeEnum = eu.nimble.service.bp.impl.clause.ClauseType.valueOf(clauseType);
            } catch (Exception e) {
                return createResponseEntityAndLog(String.format("Invalid clause type: %s. Possible values are: ", clauseType, eu.nimble.service.bp.impl.clause.ClauseType.values().toString()), e, HttpStatus.BAD_REQUEST);
            }
            ...
            if (clauseTypeEnum == eu.nimble.service.bp.impl.clause.ClauseType.ITEM_DETAILS) {
                docRef.setDocumentType(DocumentType.ITEMINFORMATIONRESPONSE.toString());
            } else if (clauseTypeEnum == eu.nimble.service.bp.impl.clause.ClauseType.NEGOTIATION) {
                docRef.setDocumentType(DocumentType.QUOTATION.toString());
            } else if (clauseTypeEnum == eu.nimble.service.bp.impl.clause.ClauseType.PPAP) {
    ...
    public ResponseEntity updateClause(@PathVariable(value = "clauseId") String clauseId,
        ...
        try {
            ...
            Object clauseObject;
            if (clause.getType().contentEquals(eu.nimble.service.bp.impl.clause.ClauseType.DATA_MONITORING.toString())) {
                try {
