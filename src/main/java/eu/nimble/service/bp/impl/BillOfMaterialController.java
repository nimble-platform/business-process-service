...
import eu.nimble.service.bp.impl.model.bom.BillOfMaterial;
...
public class BillOfMaterialController {
    ...
    @CrossOrigin(origins = {"*"})
    @ApiOperation(value = "", notes = "Creates negotiations for BOM for the given party. If there is a frame contract between parties and useFrameContract parameter is set to true, " +
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Created negotiations for BOM for the given party"),
            @ApiResponse(code = 401, message = "Invalid token. No user was found for the provided token"),
            @ApiResponse(code = 500, message = "Unexpected error while creating negotiations for BOM for the given party")
    })
    @RequestMapping(value = "/billOfMaterial",
            method = RequestMethod.POST,
            produces = {"application/json"})
    public ResponseEntity createNegotiationsForBOM(@ApiParam(value = "Serialized form of BillOfMaterial used to create request for quotations", required = true) @RequestBody String billOfMaterialJSON,
        ...
        logger.info("Creating negotiations for BOM for party: {}, useFrameContract: {}", partyId, useFrameContract);
        // check token
        ...
        try {
            // deserialize BillOfMaterial json
            BillOfMaterial billOfMaterial = JsonSerializationUtility.getObjectMapper().readValue(billOfMaterialJSON, BillOfMaterial.class);
            ...
            for (LineItemType lineItem : billOfMaterial.getLineItems()) {
            ...
            String msg = String.format("Unexpected error while creating negotiations for BOM for party : %s", partyId);
            logger.error(msg);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(msg);
        }

        logger.info("Created negotiations for BOM for party: {}, useFrameContract: {}", partyId, useFrameContract);
